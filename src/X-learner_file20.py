...
def load_data(train_or_test, treatment_or_cortonl,batch_size,shuffle):
    source_data = pd.read_excel(
        '../dataset/file20/' + str(train_or_test) + '/' + str(treatment_or_cortonl) + '.xlsx')
    source_data = np.array(source_data)
    source_data = source_data[:, :-3]

    labelpath_temp = []
    with open('../dataset/file20/' + str(train_or_test) + '/Yobs_' + str(
    ...
    Tipath_temp = []
    with open('../dataset/file20/' + str(train_or_test) + '/Ti_' + str(
            treatment_or_cortonl) + '_' + str(train_or_test) + '.txt', 'r') as f:
        while True:
            line = f.readline()
            if line:
...
def train_M0(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 100 == 0:
        ...
        dirr = '.\weight_w0\X-learner\M_FILE20_' + str(epoch) + '.pth'
...
def train_M1(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 100 == 0:
        state = {'net':model.state_dict(), ' optimizer':optimizer.state_dict(), 'epoch':epoch}
        dirr = '.\weight_w1\X-learner\M_FILE20_' + str(epoch) + '.pth'
        torch.save(state, dirr)
...
def create_D():
    ...
    source_data = pd.read_excel(
        '../dataset/file20/' + 'train' + '/' + 'treatment' + '.xlsx')
    source_data = np.array(source_data)
    source_data = source_data[:, :-3]
    source_data = torch.from_numpy(source_data)

    model0 = M()
    checkpoint = torch.load('.\weight_w0\X-learner\M_FILE20_99.pth')
    ...
    Yobs = []
    with open('../dataset/file20/train/Yobs_treatment_train.txt','r') as f:
        while True:
            line = f.readline()
            if line:
                line = line.replace('\n', '')
    ...
    output1 = Yobs - output
    with open('../dataset/file20/D1.txt','w') as f:
        for i in output1:
            f.write(str(i) + '\n')

    source_data = pd.read_excel(
        '../dataset/file20/' + 'train' + '/' + 'control' + '.xlsx')
    source_data = np.array(source_data)
    source_data = source_data[:, :-3]
    source_data = torch.from_numpy(source_data)

    model1 = M()
    checkpoint = torch.load('.\weight_w1\X-learner\M_FILE20_99.pth')
    ...
    Yobs = []
    with open('../dataset/file20/train/Yobs_control_train.txt', 'r') as f:
        while True:
            line = f.readline()
            if line:
                line = line.replace('\n', '')
    ...
    Yobs = np.reshape(Yobs, (Yobs.shape[0], 1))
    output1 = output - Yobs
    with open('../dataset/file20/D0.txt', 'w') as f:
        for i in output1:
            f.write(str(i) + '\n')
...
def load_data_D(train_or_test, treatment_or_cortonl,_0_1,batch_size,shuffle):
    ...
    labelpath_temp = []
    with open('../dataset/file20/' +  '/D' + str(
            _0_1) + '.txt', 'r') as f:
        while True:
            line = f.readline()
            if line:
...
def train_T1_detail(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 100 == 0:
        state = {'net':model.state_dict(), ' optimizer':optimizer.state_dict(), 'epoch':epoch}
        dirr = '.\weight_w1\X-learner\T_FILE20_' + str(epoch) + '.pth'
        torch.save(state, dirr)
...
def train_T0_detail(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 100 == 0:
        state = {'net': model.state_dict(), ' optimizer': optimizer.state_dict(), 'epoch': epoch}
        dirr = '.\weight_w0\X-learner\T_FILE20_' + str(epoch) + '.pth'
        torch.save(state, dirr)
...
def test1():
    ...
    checkpoint = torch.load('.\weight_w1\X-learner\T_FILE20_99.pth')
    model1.load_state_dict(checkpoint['net'])
    ...
    checkpoint = torch.load('.\weight_w0\X-learner\T_FILE20_99.pth')
    model1.load_state_dict(checkpoint['net'])

    cate0,cate01,cate02 = test_T0(model1, trainset_dataloader)

    cate_ave = 0.5 * cate0 + 0.5 * cate1
    cate_ave1 = 0.5 * cate01 + 0.5 * cate11
    cate_ave2 = 0.5 * cate02 + 0.5 * cate12
