...
def train_M0(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 100 == 0:
        ...
        dirr = '.\weight_w0\X-learner\M_GOTV_' + str(epoch) + '.pth'
...
def train_M1(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 100 == 0:
        state = {'net':model.state_dict(), ' optimizer':optimizer.state_dict(), 'epoch':epoch}
        dirr = '.\weight_w1\X-learner\M_GOTV_' + str(epoch) + '.pth'
        torch.save(state, dirr)
...
def create_D():
    ...
    source_data = pd.read_excel(
        '../dataset/gerber_huber_2014_data/' + 'train' + '/' + 'treatment' + '.xlsx')
    source_data = np.array(source_data)
    race_data = source_data[:, 10:-1]
    age = source_data[:, 9]
    age = age / np.mean(age)
    ...
    model0 = M()
    checkpoint = torch.load('.\weight_w0\X-learner\M_GOTV_99.pth')
    ...
    Yobs = []
    with open('../dataset/gerber_huber_2014_data/train/Yobs_treatment_train.txt','r') as f:
        while True:
            line = f.readline()
            if line:
                line = line.replace('\n', '')
    ...
    output1 = Yobs - output
    with open('../dataset/gerber_huber_2014_data/D1.txt','w') as f:
        for i in output1:
            f.write(str(i) + '\n')

    source_data = pd.read_excel(
        '../dataset/gerber_huber_2014_data/' + 'train' + '/' + 'control' + '.xlsx')
    source_data = np.array(source_data)
    race_data = source_data[:, 10:-1]
    age = source_data[:, 9]
    age = age / np.mean(age)
    ...
    model1 = M()
    checkpoint = torch.load('.\weight_w1\X-learner\M_GOTV_99.pth')
    ...
    Yobs = []
    with open('../dataset/gerber_huber_2014_data/train/Yobs_control_train.txt', 'r') as f:
        while True:
            line = f.readline()
            if line:
                line = line.replace('\n', '')
    ...
    Yobs = np.reshape(Yobs, (Yobs.shape[0], 1))
    output1 = output - Yobs
    with open('../dataset/gerber_huber_2014_data/D0.txt', 'w') as f:
        for i in output1:
            f.write(str(i) + '\n')
...
def load_data_D(train_or_test, treatment_or_cortonl,_0_1,batch_size,shuffle):
    ...
    labelpath_temp = []
    with open('../dataset/gerber_huber_2014_data/' +  '/D' + str(
            _0_1) + '.txt', 'r') as f:
        while True:
            line = f.readline()
            if line:
...
def train_T1_detail(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 100 == 0:
        state = {'net':model.state_dict(), ' optimizer':optimizer.state_dict(), 'epoch':epoch}
        dirr = '.\weight_w1\X-learner\T_GOTV_' + str(epoch) + '.pth'
        torch.save(state, dirr)
...
def train_T0_detail(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 100 == 0:
        state = {'net': model.state_dict(), ' optimizer': optimizer.state_dict(), 'epoch': epoch}
        dirr = '.\weight_w0\X-learner\T_GOTV_' + str(epoch) + '.pth'
        torch.save(state, dirr)
...
def test1():
    ...
    checkpoint = torch.load('.\weight_w1\X-learner\T_GOTV_99.pth')
    model1.load_state_dict(checkpoint['net'])
    ...
    checkpoint = torch.load('.\weight_w0\X-learner\T_GOTV_99.pth')
    model1.load_state_dict(checkpoint['net'])
