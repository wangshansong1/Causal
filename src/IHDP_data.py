...
def DGP_1(train_or_test,treatment_or_cortonl, epsilon_u0, epsilon_u1):
    ...
    inX_u0 = np.dot(source_data, epsilon_u0.reshape(25, 1))
    inX_u1 = np.dot(source_data, epsilon_u1.reshape(25, 1))
...
def DGP_2():
    while True:
        epsilon_u0 = np.random.normal(loc=0, scale=1, size=25)
        epsilon_u1 = np.random.normal(loc=0, scale=1, size=25)
    ...
    with open('Ti_treatment_train.txt','w') as f:
        for i in Ti_tr:
            if i < 0:
                ...
                i = np.random.binomial(1, i, size=1)
            f.write(str(i) + '\n')
