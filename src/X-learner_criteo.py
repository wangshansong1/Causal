...
def load_data(train_or_test, treatment_or_cortonl,batch_size,shuffle):
    source_data = pd.read_excel(
        '../dataset/criteo/' + str(train_or_test) + '/' + str(treatment_or_cortonl) + '.xlsx')
    source_data = np.array(source_data)
    source_data = source_data[:, :-4]

    labelpath_temp = []
    with open('../dataset/criteo/' + str(train_or_test) + '/Yobs_' + str(
    ...
    Tipath_temp = []
    with open('../dataset/criteo/' + str(train_or_test) + '/Ti_' + str(
            treatment_or_cortonl) + '_' + str(train_or_test) + '.txt', 'r') as f:
        while True:
            line = f.readline()
            if line:
...
def train_M0(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 10 == 0:
        ...
        dirr = '.\weight_w0\X-learner\M_CRITEO_' + str(epoch) + '.pth'
...
def train_M1(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 10 == 0:
        state = {'net':model.state_dict(), ' optimizer':optimizer.state_dict(), 'epoch':epoch}
        dirr = '.\weight_w1\X-learner\M_CRITEO_' + str(epoch) + '.pth'
        torch.save(state, dirr)
...
def create_D():
    ...
    source_data = pd.read_excel(
        '../dataset/criteo/' + 'train' + '/' + 'treatment' + '.xlsx')
    source_data = np.array(source_data)
    source_data = source_data[:, :-4]
    source_data = torch.from_numpy(source_data)

    model0 = M()
    checkpoint = torch.load('.\weight_w0\X-learner\M_CRITEO_9.pth')
    ...
    Yobs = []
    with open('../dataset/criteo/train/Yobs_treatment_train.txt','r') as f:
        while True:
            line = f.readline()
            if line:
                line = line.replace('\n', '')
    ...
    output1 = Yobs - output
    with open('../dataset/criteo/D1.txt','w') as f:
        for i in output1:
            f.write(str(i) + '\n')

    source_data = pd.read_excel(
        '../dataset/criteo/' + 'train' + '/' + 'control' + '.xlsx')
    source_data = np.array(source_data)
    source_data = source_data[:, :-4]
    source_data = torch.from_numpy(source_data)

    model1 = M()
    checkpoint = torch.load('.\weight_w1\X-learner\M_CRITEO_9.pth')
    ...
    Yobs = []
    with open('../dataset/criteo/train/Yobs_control_train.txt', 'r') as f:
        while True:
            line = f.readline()
            if line:
                line = line.replace('\n', '')
    ...
    Yobs = np.reshape(Yobs, (Yobs.shape[0], 1))
    output1 = output - Yobs
    with open('../dataset/criteo/D0.txt', 'w') as f:
        for i in output1:
            f.write(str(i) + '\n')
...
def load_data_D(train_or_test, treatment_or_cortonl,_0_1,batch_size,shuffle):
    ...
    labelpath_temp = []
    with open('../dataset/criteo/' +  '/D' + str(
            _0_1) + '.txt', 'r') as f:
        while True:
            line = f.readline()
            if line:
...
def train_T1_detail(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 10 == 0:
        state = {'net':model.state_dict(), ' optimizer':optimizer.state_dict(), 'epoch':epoch}
        dirr = '.\weight_w1\X-learner\T_CRITEO_' + str(epoch) + '.pth'
        torch.save(state, dirr)
...
def train_T0_detail(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 10 == 0:
        state = {'net': model.state_dict(), ' optimizer': optimizer.state_dict(), 'epoch': epoch}
        dirr = '.\weight_w0\X-learner\T_CRITEO_' + str(epoch) + '.pth'
        torch.save(state, dirr)
...
def test1():
    ...
    checkpoint = torch.load('.\weight_w1\X-learner\T_CRITEO_9.pth')
    model1.load_state_dict(checkpoint['net'])
    ...
    checkpoint = torch.load('.\weight_w0\X-learner\T_CRITEO_9.pth')
    model1.load_state_dict(checkpoint['net'])
