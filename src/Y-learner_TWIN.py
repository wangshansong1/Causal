...
class Y_Learner(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc2_1 = nn.Linear(50, 5)
        self.fc2_2 = nn.Linear(5, 1)

        self.fc3_1 = nn.Linear(50, 5)
...
def train_W0(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 100 == 0:
        ...
        dirr = '.\weight_w0\Y-learner\TWIN_' + str(epoch) + '.pth'
...
def train_W1(model, trainset_dataloader, optimizer, epoch):
    ...
    if (epoch + 1) % 100 == 0:
        state = {'net':model.state_dict(), ' optimizer':optimizer.state_dict(), 'epoch':epoch}
        dirr = '.\weight_w1\Y-learner\TWIN_' + str(epoch) + '.pth'
        torch.save(state, dirr)
...
def test1():
    ...
    checkpoint = torch.load('.\weight_w1\Y-learner\TWIN_99.pth')
    ...
    checkpoint = torch.load('.\weight_w0\Y-learner\TWIN_99.pth')
    model1.load_state_dict(checkpoint['net'])
