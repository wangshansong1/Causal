...
def DGP_1(train_or_test,treatment_or_cortonl, epsilon_u0, epsilon_u1):
    ...
    source_data = pd.read_excel('../dataset/criteo/' + str(train_or_test) +'/'+ str(treatment_or_cortonl) + '.xlsx')
    source_data = np.array(source_data)
    source_data = source_data[:, :-4]

    inX_u0 = np.dot(source_data, epsilon_u0.reshape(12, 1))
    inX_u1 = np.dot(source_data, epsilon_u1.reshape(12, 1))
...
def DGP_2():
    # while True:
    epsilon_u0 = np.random.normal(loc=0, scale=1, size=12)
    epsilon_u1 = np.random.normal(loc=0, scale=1, size=12)
